import { daysBetween } from '@project/shared/progress';
...
export function dueLabel(day: string | null | undefined): { label: string; tone: DueTone; days: number } | null {
  if (!day) return null;
  // Counted in UTC calendar days, the same basis dueState() uses, so a label never disagrees with its colour.
  const days = daysBetween(new Date().toISOString().slice(0, 10), day);
